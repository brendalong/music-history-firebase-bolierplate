...
function loadSongsToDOM() {
  ...
  db.getSongs(currentUser).then((songData)=> {
    console.log("I got songs buddy", songData);
...
$(document).on("click", ".save_new_btn", function() {
  ...
  db.addSong(songObj).then((songID) => {
...
$(document).on("click", ".save_edit_btn", function() {
  ...
  db.editSong(songObj, songID).then((date) => {
    loadSongsToDOM();
  });
});
...
$("#auth-btn").click(function(){
  ...
  login.logInGoogle().then((result) => {
    console.log("results from login", result.user.uid);
